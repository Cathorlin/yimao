...
public class BarCodeHelper
{
    ...
    public static Image DrawImg39(string Value,int BarWeight,int Height, bool AddQuietZone)
    {
        int x = 0; //左边界
        int y = 0; //上边界
        int WidLength = 2 * BarWeight; //粗BarCode长度
        int NarrowLength = 1 * BarWeight; //细BarCode长度
        int BarCodeHeight = Height; //BarCode高度

        if (AddQuietZone)
        {
            BarCodeHeight = BarCodeHeight + 20;
        }


        String Encoded_Value = Encode_Code39(Value);
        int intSourceLength = Value.Length;
        Bitmap objBitmap = new Bitmap(((WidLength * 3 + NarrowLength * 7) * (intSourceLength + 2)) + (x * 2), BarCodeHeight );
      //  Graphics objGraphics = Graphics.FromImage(objBitmap);
        using (Graphics objGraphics = Graphics.FromImage(objBitmap))
        {
            ...
            for (int i = 0; i < intEncodeLength; i++) //依码Code39 BarCode
            {
                ...
                objGraphics.FillRectangle(i % 2 == 0 ? Brushes.Black : Brushes.White, x, y, intBarWidth, Height);
                x += intBarWidth;
            }
            if (AddQuietZone)
            {
                objGraphics.DrawString(Value, new Font("Arial", 9), Brushes.Black, new PointF(2, BarCodeHeight - 20));
            }
        }
        return objBitmap;
    }
    public static string Encode_Code39(string Raw_Data_)
    {
        ...
        string Raw_Data = Raw_Data_.ToUpper();
