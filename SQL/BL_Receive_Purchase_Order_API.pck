...
/*      SELECT  COL01 INTO ROWLISTOLD_ FROM AF  WHERE COL='Move_Receipt1' ;*/
 ...
 PROCEDURE In_Move_Receipt(ROWLIST_ VARCHAR2,
  ...
  receive_vdate_ varchar2(10);
  --receive_date_ date;
 --  ROWLIST1_  varchar2(4000);
 begin 
/*   insert into af(col,col01)
   select 'frank1',ROWLIST_ from dual ;
   commit;*/
   
  --  select col01 into  ROWLIST1_ from af  where col ='frank1';
   row_.objid := pkg_a.Get_Item_Value('OBJID',ROWLIST_);
    open  cur_ for 
   Select t.*
...
--modify fjp 2013-02-04
   ...
   ---end----
    ...
    --end  
    --modify fjp 2013-02-04 更改事物日期为入库日期  2013-03-11 不更改入库日期
/*       if   receive_vdate_ is not null then 
           open  cur_ for 
            select t.* 
            ...
            WHILE  cur_%FOUND LOOP
              Inventory_Transaction_Hist_API.Modify_Date_Applied(ROWST_.transaction_id,
                                                                 to_date(receive_vdate_,'yyyy-mm-dd'));
              FETCH cur_ INTO rowst_; 
            END LOOP;
            CLOSE cur_;   
        end if;  */                                          
