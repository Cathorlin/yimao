CREATE OR REPLACE PACKAGE BL_TRANSPORT_NOTEC_API IS
  ...
  FUNCTION CHECK_CONTRACT(ROWLIST_ IN VARCHAR2) RETURN BOOLEAN;
  FUNCTION CHECK_DUPLICATE(CONTRACT_ IN VARCHAR2, PICKLISTNO_ IN VARCHAR2)
    RETURN BOOLEAN;
     PROCEDURE CLOSE__(ROWLIST_  VARCHAR2,
                    USER_ID_  VARCHAR2,
                    A311_KEY_ VARCHAR2);
END BL_TRANSPORT_NOTEC_API;
/
CREATE OR REPLACE PACKAGE BODY BL_TRANSPORT_NOTEC_API IS
  TYPE T_CURSOR IS REF CURSOR;
  /*  新增初始化 New__
  Rowlist_ 初始化的参数 可以传入requseturl 当前请求的url地址
  User_Id_  当前用户
  A311_Key_ A314的主键 
  modify 2013-01-06 fjp
  modify 2013-01-07 fjp 取消确认*/
...
--右键功能取消--
  ...
  BEGIN
    OPEN CUR_ FOR
      SELECT T.* FROM BL_V_TRANSPORT_NOTEC T WHERE T.OBJID = ROWLIST_;
    FETCH CUR_
      INTO ROW_;
    ...
    PKG_A.SETMSG(A311_KEY_, '', '快递发货' || ROW_.NOTE_NO || '取消成功！');
    RETURN;
  END;
   --右键关闭--
  PROCEDURE CLOSE__(ROWLIST_  VARCHAR2,
  ...
  BEGIN
    ...
    IF CUR_%NOTFOUND THEN
      CLOSE CUR_;
      PKG_A.SETFAILED(A311_KEY_, 'BL_V_TRANSPORT_NOTEC', ROWLIST_);
      RAISE_APPLICATION_ERROR(-20101, '错误的rowid');
      RETURN;
    END IF;
    CLOSE CUR_;
    ...
    PKG_A.SETMSG(A311_KEY_, '', '快递发货关闭成功');
...
/*      SELECT T.ROWID
 ...
 /*     insert into BL_TRANSPORT_NOTECONTRACT(  note_no   ,
                                              ...
                                              containerno  ,
                                              state )
       SELECT ROW_.NOTE_NO,supplier,'0',ROW_.STATE
          from BL_V_DISTPICKLIST_CON b
    ...
    IF DOACTION_ = 'M' THEN
      -- 更改数据
      OPEN CUR_ FOR
        SELECT T.* FROM BL_V_TRANSPORT_NOTEC T WHERE T.OBJID = OBJID_;
      FETCH CUR_
        INTO ROW_;
      IF CUR_%NOTFOUND THEN
        CLOSE CUR_;
...
/*      ELSE
        IF ROW_.STATE > 0 THEN
          PKG_A.SETMSG(A311_KEY_,
                       '',
                       '快递送货' || ROW_.NOTE_NO || '已确认，不可修改');
          CLOSE CUR_;
          RETURN;
        END IF;*/
      END IF;
      CLOSE CUR_;
    --作废2013-01-25 modify fjp 
/*      PICKLISTNO_ := PKG_A.GET_ITEM_VALUE('PICKLISTNO', ROWLIST_);
      ...
      LOOP
        ...
        IF COLUMN_ID_ <> 'OBJID' AND COLUMN_ID_ <> 'DOACTION' AND
           COLUMN_ID_ <> 'CONTRACTDESC' AND COLUMN_ID_ <> 'AIMPORTDESC' AND
  ...
  BEGIN
    ...
    END IF;
    CLOSE CUR_;
    IF CHECK_CONTRACT(ROWLIST_) = FALSE THEN
      RAISE_APPLICATION_ERROR(-20101, '备货单的域不匹配');
    END IF;
  
    --确定时要是备货单号有多个域，要把另几个也改掉--
 ...
 /*   OPEN CUR_ FOR
    ...
    UPDATE BL_TRANSPORT_NOTE SET STATE = '1' WHERE ROWID = ROW_.OBJID;
    update BL_TRANSPORT_NOTECONTRACT  set  state='1' where NOTE_NO= row_.NOTE_NO;
    PKG_A.SETMSG(A311_KEY_, '', '快递送货下达成功');
...
/*  
    ...
    UPDATE BL_TRANSPORT_NOTE SET STATE = '0' WHERE ROWID = ROW_.OBJID;
    update BL_TRANSPORT_NOTECONTRACT  set  state='0' where NOTE_NO= row_.NOTE_NO;
    PKG_A.SETMSG(A311_KEY_, '', '快递送货取消下达成功');
  ...
  */
  ...
  BEGIN
    ...
    END IF ;
    IF ROW_.STATE != 0 THEN
      IF COLUMN_ID_ = 'CONTRACT' OR COLUMN_ID_ = 'CONTRACTDESC' OR
         COLUMN_ID_ = 'SHIPDATE' OR COLUMN_ID_ = 'SHIPTIME' OR
         COLUMN_ID_ = 'SHIPPINGMARK' OR COLUMN_ID_ = 'REMARK' OR
         COLUMN_ID_ = 'PICKLISTNO' --OR COLUMN_ID_ = 'EXPRESSID' 
       THEN
        RETURN '0';
      ELSE
        RETURN '1';
  ...
  FUNCTION CHECK_CONTRACT(ROWLIST_ IN VARCHAR2) RETURN BOOLEAN IS
    ROW_B  BL_V_TRANSPORT_NOTEC%ROWTYPE;
    CUR_B  T_CURSOR;
    COUNT1 NUMBER;
    COUNT2 NUMBER;
  BEGIN
    OPEN CUR_B FOR
      SELECT * FROM BL_V_TRANSPORT_NOTEC T WHERE T.OBJID = ROWLIST_;
    FETCH CUR_B
      INTO ROW_B;
    ...
    END IF;
    CLOSE CUR_B;
 /*   SELECT T1.PICKLISTNO
      INTO PICKLISTNO_
      FROM BL_TRANSPORT_NOTE T1
     WHERE T1.ROWID = ROWLIST_;
    ...
    SELECT COUNT(DISTINCT(T.CONTRACT))
      INTO COUNT1
      FROM BL_V_TRANSPORT_NOTECONTRACTC T
     WHERE T.NOTE_NO = ROW_B.NOTE_NO;
  
    SELECT COUNT(DISTINCT(T.SUPPLIER))
      INTO COUNT2
