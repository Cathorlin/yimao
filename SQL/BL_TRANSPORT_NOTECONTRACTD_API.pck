CREATE OR REPLACE PACKAGE BL_TRANSPORT_NOTECONTRACTD_API IS
  /*  新增初始化 New__
...
END BL_TRANSPORT_NOTECONTRACTD_API;
/
CREATE OR REPLACE PACKAGE BODY BL_TRANSPORT_NOTECONTRACTD_API IS
...
If Doaction_ ='D'Then
   /*OPEN CUR_ FOR
        SELECT T.* FROM BL_V_TRANSPORT_NOTECONTRACTD T WHERE T.ROWID = OBJID_;
      FETCH CUR_
        INTO ROW_;
      IF CUR_ %NOTFOUND THEN
        CLOSE CUR_;
      ...
      close cur_;
--      DELETE FROM BL_V_TRANSPORT_NOTECONTRACTD T WHERE T.ROWID = OBJID_; */
--pkg_a.Setsuccess(A311_Key_,'BL_V_TRANSPORT_NOTECONTRACTD', Objid_);
Return;
End If;
