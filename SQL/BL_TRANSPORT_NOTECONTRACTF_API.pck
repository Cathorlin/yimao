CREATE OR REPLACE PACKAGE BL_TRANSPORT_NOTECONTRACTF_API IS
  /*  新增初始化 New__
...
END BL_TRANSPORT_NOTECONTRACTF_API;
/
CREATE OR REPLACE PACKAGE BODY BL_TRANSPORT_NOTECONTRACTF_API IS
  ...
  */
  PROCEDURE Modify__(Rowlist_  VARCHAR2,
    ...
    Data_      Varchar(4000);
    Mysql_     Varchar(4000);
    Ifmychange Varchar(1); row_      BL_V_TRANSPORT_NOTECONTRACTF%rowtype;
    BEGIN  
...
If Doaction_ ='D'Then
   /*OPEN CUR_ FOR
        SELECT T.* FROM BL_V_TRANSPORT_NOTECONTRACTF T WHERE T.ROWID = OBJID_;
      FETCH CUR_
        INTO ROW_;
      IF CUR_ %NOTFOUND THEN
        CLOSE CUR_;
      ...
      close cur_;
--      DELETE FROM BL_V_TRANSPORT_NOTECONTRACTF T WHERE T.ROWID = OBJID_; */
--pkg_a.Setsuccess(A311_Key_,'BL_V_TRANSPORT_NOTECONTRACTF', Objid_);
Return;
End If;
