...
function setDayHM(obj) {
    ...
    if (tiannetOutObject != null && tiannetOutObject != obj) {
        tiannetHideControl();
    }
 tiannetOutObject = obj;
 m_bolShowHour = true;
 ...
 var cdate = ChkData(strValue) ;

  tYear = cdate.getFullYear(); //定义年的变量的初始值
  tMonth = cdate.getMonth() + 1; //定义月的变量的初始值
  tDay =  cdate.getDate();
  tHour = cdate.getHours();
  tMinute = cdate.getMinutes();
  if (tMonth < 10) tMonth = '0' + tMonth;
  if (tDay < 10) tDay = '0' +  tDay;
  if (tHour < 10) tHour = '0' +  tHour;
  if (tMinute < 10) tMinute = '0' + tMinute;
  strValue = tYear + "-" + tMonth + "-" + tDay + " " + tHour + ":" + tMinute + ":00";
...
function ChkData(str) {
    ...
    if (r == null) {
        ...
        return cdate;
        r = new Array();
        r[1] = cdate.getFullYear(); //定义年的变量的初始值
        r[2] = cdate.getMonth(); //定义月的变量的初始值
        r[3] = cdate.getDate();
        r[4] = cdate.getHours();
        r[5] = cdate.getMinutes();
        r[6] = 0;
    }
    else {
        r[2] = r[2] - 1;
