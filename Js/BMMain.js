...
function getmyheight() {
    try {
        myheight = parent.get_mainheight();
    } catch (e) {
        myheight = 0;
    }
...
function createTree(treeid_, zNodes_) {
    showNodeCount = 0;
    //$("#treeDemo").empty();
    $("#" + treeid_ +"ul").empty();
    $.fn.zTree.init($("#" + treeid_ + "ul"), setting, zNodes_);
    $("#" + treeid_ + "ul").width($("#" + treeid_).width() - 15 );
    $("#" + treeid_ + "ul").height($("#showtree").height()- 15);
    $("#" + treeid_ + "ul").css("margin", "0 0 0 0 ");
    $("#showtree").css("overflow", "hidden");
}
var showNodeCount = 0;
...
function show_tree(width_) {
   // document.getElementById("mleft").style.width = width_;
    ...
    $("#mright").width(rwidth);
    $("#showdiv").width(rwidth);
    $("#mleft").height($("#mright").height());
    $("#showtree").height($("#mright").height());
        ...
        if (ilist[i].contentWindow.location.href == url) {
            ...
            if (sdiv != null) {
                ...
                if (a002_key_ == a002key) {
                    var divid = "div_req";
                    var parm_ = parm.replace("[TREEID]", sdiv.id);
                    // FunGetHttp(html_, sdiv.id, parm);
                    FunGetHttp(html_, divid, parm_);
                   // FunGetHttp(html_, sdiv.id, parm);
                }
            }            
            break;
...
function CreateTab(url, menu_id, menu_name) {
    ...
    if (lb_exist == false) {
        ...
        showframe.src = url;
        showframe.style.width = String($("#showdiv").width() )  +  "px";
        showframe.style.height =  String($("#showdiv").height()) + "px";
...
//        var div_div_ = document.createElement("div");
//        div_div_.id = "d" + menu_id;
//        div_div_.innerHTML = "ddd" + menu_id;
//        div_.appendChild(div_div_);
        ...
        li_.innerHTML = '<div id="w_' + menu_id + '" menu_id="' + menu_id + '" menu_name ="' + menu_name + '" shortname="' + menu_name + '"><a title="' + menu_name + '" alt="' + menu_name + '" onclick="show__(\'' + menu_id + "','" + menu_name + "')\"><span>" + showname + '<b class="close_m" onclick="closeshow__(\'' + menu_id + "','" + menu_name + '\')" mousemove="mv(this,\'m\')" onmouseout="mv(this,\'o\')" title="关闭">&nbsp; &nbsp;</b></a></div>';
        ...
        mObj.innerHTML = "<ul id=\"tree_" + menu_id + "ul\" class=\"ztree\"></ul>";
...
function setwidth() {
    ...
    for (var i = 0; i < ilist.length; i++) {
        ...
        obj.innerHTML = '<a title="' + menu_name + '" alt="' + menu_name + '" onclick="show__(\'' + menu_id + "','" + menu_name + "')\"><span>" + menu_name + '<b class="close_m" onclick="closeshow__(\'' + menu_id + "','" + menu_name + '\')" mousemove="mv(this,\'m\')" onmouseout="mv(this,\'o\')" title="关闭">&nbsp; &nbsp; </b></a>';
    ...
    if (w > allw) {
        ...
        for (var i = 0; i < ilist.length; i++) {
            ...
            obj.innerHTML = '<a title="' + menu_name + '" alt="' + menu_name + '" onclick="show__(\'' + menu_id + "','" + menu_name + "')\"><span>" + thisshortname + '<b class="close_m" onclick="closeshow__(\'' + menu_id + "','" + menu_name + '\')" mousemove="mv(this,\'m\')" onmouseout="mv(this,\'o\')" title="关闭">&nbsp; &nbsp; </b></a>';
        }
    }
}
